...
Auto-generated by `scripts/render-layouts.js` from `src/default.vil`. Edit this file and run `npm run apply` to write your changes back to `src/default.vil` (and rebuild `dist/default.vil`) — or edit `src/default.vil` directly and re-run `npm run render` to regenerate this file from scratch.
