...
Auto-generated by `scripts/render-layouts.js` from `src/dvorak.vil`. Edit this file and run `npm run apply` to write your changes back to `src/dvorak.vil` (and rebuild `dist/dvorak.vil`) — or edit `src/dvorak.vil` directly and re-run `npm run render` to regenerate this file from scratch.
