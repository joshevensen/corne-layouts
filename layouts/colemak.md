...
Auto-generated by `scripts/render-layouts.js` from `src/colemak.vil`. Edit this file and run `npm run apply` to write your changes back to `src/colemak.vil` (and rebuild `dist/colemak.vil`) — or edit `src/colemak.vil` directly and re-run `npm run render` to regenerate this file from scratch.
